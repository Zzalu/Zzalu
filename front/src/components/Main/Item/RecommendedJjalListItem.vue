...
export default {
  ...
  setup() {
    ...
    const send_select_recommend_gif_id_data = (e) => {
  ...
  computed: {
    img_select() {
      if (this.i == this.select_recommend_jjal_num) {
        return true;
      } else {
        return false;
      }
...
.recommend-img {
  @apply w-28 h-36 rounded-2xl mr-2 border bg-cover bg-no-repeat bg-center border-zz-dark-div;
}
